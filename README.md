...
Lucy Studio는 GUI 전용 데스크톱 툴이라 직접 조작은 직접 진행했고, `docs.edencrew.com` Script API 문서를 바탕으로 위젯 구성과 스크립트 가이드를 AI를 사용해 정리해서 대화하는 방식으로 함께 만들었습니다. 페이지는 `targetAlert`(메인) / `targetAlertDialog`(등록 다이얼로그) 두 개로 구성했습니다.

**필수 항목**: 헤더 + `+` 버튼, 등록된 목록(종목명/목표가/구분선), 빈 상태, 등록 다이얼로그(라벨 · placeholder · 매도/매수 · 배경 딤 · x 닫기), 실제 등록/반영, 빈 값 검증(에러 문구 + 빨간 보더)까지 모두 구현했습니다.

**직접 판단한 부분과 이유**

- **리스트 행을 Component로 분리하지 못했습니다.** `ListView.setState(index, callback)`으로 각 아이템의 Component 노출 변수를 갱신하는 방식을 시도했지만, 이 빌드에서 콜백 스코프 안에서 컴포넌트 인스턴스에 안정적으로 접근하는 방법을 확인하지 못했습니다(문서에도 정확한 스코프 규칙이 명시돼 있지 않았습니다). 시간 제약상 안정성을 위해 ListView 아이템 템플릿에 `Text` 위젯(id로 직접 접근)을 바로 넣는 방식으로 전환했습니다. 재사용 가능한 컴포넌트 분리라는 요구사항은 충족하지 못했지만, 등록 기능 자체는 확실하게 동작합니다.
- **커스텀 폰트(Noto Sans KR)를 적용하지 못했습니다.** Lucy Studio의 Style 패널에 커스텀 폰트 파일을 업로드/등록하는 경로를 문서와 에디터 양쪽에서 확인하지 못해, 기본 제공 서체로 진행했습니다. 색상 · 간격 토큰은 `lib/theme/README.md` 대응표의 hex 값을 그대로 사용했습니다.
- **엔진 동작 관련 메모**: 위젯의 `text`/`style.color` 같은 속성은 `setProperty("prop", value)`로 잘 반영됐지만, `visible`만은 같은 방식이 먹히지 않아 `widget.visible = true/false` 직접 할당으로 바꿔서 해결했습니다. 다이얼로그를 닫을 때도 `$form.closeDialog(result)`가 반응하지 않아 `$form.close(result)`로 바꿔서 해결했습니다. 폼 변수(`$form.getVar`/`setVar`)로 목록을 누적하는 것도 값이 유지되지 않는 문제가 있어, 스크립트 최상단의 일반 배열 변수로 대체했습니다.

완료 후 `cloneProject/assets`를 압축했습니다.
